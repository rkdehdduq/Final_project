...
def classify_sector(user_input, user_profile):
    ...
    print("정제된 sector_list:", sector_list)  # 디버깅용 출력
...
def riskfolio(topfund, corfunds):
    ...
    if '수익률' not in recent_6_months.columns or recent_6_months.empty:
        return("수익률 데이터가 충분하지 않거나 유효하지 않습니다. 필터링 후 남아있는 데이터가 없습니다.")
    ...
    if fund_returns_6_months.empty:
        return("수익률 데이터가 충분하지 않습니다. 필터링 후 남아있는 데이터가 없습니다.")
    ...
    w_min = [0.01] * len(funds)
    w_max = [0.5] * len(funds)  # 모든 자산의 최대 가중치 50%로 설정
...
#    port.bounds = (0.05, 0.5)
    ...
    if w is None or 'weights' not in w:
        return("최적화 결과가 유효하지 않습니다. 포트폴리오 가중치를 계산할 수 없습니다.")
    print(w)

    min_required_weight = 0.4  # 최소 요구 가중치 (40%)
    if w.loc[topfund, 'weights'] < min_required_weight:
        # 부족한 가중치를 0.4로 설정
        w.loc[topfund, 'weights'] = min_required_weight

        # 나머지 자산의 가중치를 합이 1 - 0.67 = 0.33이 되도록 재조정
        other_funds = w[w.index != topfund]
        other_funds_total_weight = other_funds['weights'].sum()

        # 나머지 자산들의 가중치를 비율에 따라 재조정
        w.loc[other_funds.index, 'weights'] = other_funds['weights'] * (1 - min_required_weight) / other_funds_total_weight
    print(w)
    w = w[w['weights'] > 0.01]
    print(w)
...
def extract_monthly_dates_from_question(question):
    ...
    if not date_info or ',' not in date_info:
        ...
        startdate = (datetime.strptime(latest_data_month, '%Y-%m') - relativedelta(months=12)).strftime('%Y-%m')  # 12개월 전
    else:
        # 날짜 형식이 올바른지 확인하고 분할
...
def generate_price_graph(fund_info, startdate, enddate):
  ...
  # 데이터 준비
    ...
    scales = (filtered_data['설정원본'] * filtered_data['기준가격'] / 1e6).tolist()  # 규모 계산 (설정원본 * 기준가격)
    ...
    # 설정액 축 설정 (파랑색 라인)
    ax2 = ax1.twinx()
    ax2.plot(dates, scales, marker='s', color='#4c72ff', linestyle='--', linewidth=2, markersize=6, label="설정액(백만 원)")
    ax2.set_ylabel("설정액 (백만 원)", fontsize=14, color='#4c72ff')
    ax2.tick_params(axis='y', labelcolor='#4c72ff')

    # 설정액 축 포맷팅
    ax2.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f'{x:,.1f}'))  # 백만 단위로 표시

    # 제목과 스타일 설정
    plt.title(f"{startdate} ~ {enddate} 펀드 기준가격 및 규모 추이", fontsize=16, fontweight='bold', color='#333333')
    plt.xticks(fontsize=9, rotation=90, ha='center', color='#333333')
    plt.tight_layout()  # 그래프 요소 간격을 자동으로 조정
    ...
    messages=[
            {
                ...
                "content": (
                    ...
                    "1. 섹터별 펀드 추천 (R): '추천'이라는 단어가 들어가거나, 특정 섹터를 기준으로 펀드 추천을 요청하는 경우. 기본 섹터는 {base_sectors}\n"
                    "2. 펀드 명을 통한 펀드 정보 출력 (F): 질문에 특정 펀드명이 포함된 경우. 펀드 명은 고유명사야. 기본 섹터만을 물어보는 경우는 해당되지 않아."
                    "예시: '미래에셋퇴직연금미국리츠40증권자투자신탁에 대한 펀드 정보 알려줄래'.\n"
                    "3. 그 외 경제 관련 질문이 아닌 경우 (N).\n"
                    "펀드 명을 포함한 질문은 항상 F로 분류하고, 섹터를 포함하지 않은 추천을 원할 경우 (N)으로 분류해줘. 섹터를 포함한 추천은 모두 R로 분류해줘 섹터에는 tdf도 있어"
                )
            },
...
def standard_chatbot(user_input):
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "사용자의 질문이 들어오면 gpt의 기능을 활용하여 잘 대답해주지만 펀드에 관한 질문을 해달라고 유도해. 펀드나 투자 관련 질문(용어 설명 등)이라면 대답해줘. 임의로 펀드를 추천하지 말고 섹터 또는 펀드명을 유도해."},
...
def generate_final_response(user_grade, sector, top_fund, weight):
    ...
    messages=[
        ...
        f"또한 {top_fund}와 음의 상관관계를 가진 다른 펀드들의 펀드 명과 가중치 비율이 기록된 {weight}가 있습니다. "
        f"이 정보를 바탕으로 사용자에게 최종 펀드 포트폴리오 추천 메시지를 간결하게 생성해주세요."
...
def generate_specific_fund_info_response(user_question, fund_info):
    ...
    prompt = f"""
        ...
        그에 맞는 데이터만 반환하세요.(최대 1년) 1년 이상을 요청한다면 해당 데이터는 그래프를 참고하라고 답변해주세요. 최신 데이터는 2024년 9월입니다. 최신 데이터를 기반으로 답해주세요\
        기본적으로는 투자목적, 주요 섹터, 국가, 현재일자 기준 최근 12개월 수익률, 투자설명서 및 펀드 정보 다운로드 경로 등의 펀드의 기본 정보와 함께 따로 만들어진 그래프를 기본으로 제공합니다.


    펀드의 기본 정보:
    - 펀드명: {fund_info.get('펀드명', '정보 없음')}
    - 투자 목적: {fund_info.get('투자 목적', '정보 없음')}
    - 주요 섹터: {fund_info.get('주요 섹터', '정보 없음')}
    - 국가: {fund_info.get('주요 지역', '정보 없음')}
    - 수익률 정보: {all_returns_str}
    - 투자설명서 및 펀드 정보 다운로드 링크: {fund_info.get('폴더경로', '정보 없음')}
...
def chat_with_gpt(user_input, user_profile):
    ...
    if classification == "R":
        ...
        if sector == 0:
            ...
            return "어떤 채권 펀드를 원하시나요? 단기채권, 중기채권, 장기채권, MMF, 하이일드 등이 있어요."
        else:
